...
describe("Paper Management API Tests", () => {
  ...
  // PUT /api/papers/:id
describe("PUT /api/papers/:id", () => {
  it("should update an existing paper when fields change", async () => {
    ...
    const res = await request(app).put(`/api/papers/${createRes.body.id}`).send(updatedPaper);
  ...
  it("should return 200 and not update when fields are the same", async () => {
    const createRes = await request(app).post("/api/papers").send(samplePaper);

    const res = await request(app).put(`/api/papers/${createRes.body.id}`).send(samplePaper);
  ...
  it("should return 400 if some required fields are missing", async () => {
    const createRes = await request(app).post("/api/papers").send(samplePaper);
    const incompleteUpdate = { title: "New Title" }; // Missing authors, published_in, year

    const res = await request(app).put(`/api/papers/${createRes.body.id}`).send(incompleteUpdate);

    expect(res.status).toBe(400);
    expect(res.body.error).toBe("Validation Error");
    expect(res.body.messages).toContain("Authors are required");
  ...
  it("should return 404 if paper ID does not exist", async () => {
    const res = await request(app).put("/api/papers/99999").send(samplePaper);
  ...
  it("should return 400 if ID is invalid", async () => {
    const res = await request(app).put("/api/papers/invalid-id").send(samplePaper);
